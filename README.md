...
PHP 코드 인덱싱 및 대시보드 - **🔧 색인 관리가 기본 화면**
...
# 기본 화면: 🔧 색인 관리 (v2.4.1+)
- ✅ 색인 상태 조회: 파일 크기, 심볼 수, 생성 시간, 커버리지
- ⚙️ 재색인: 강제 재색인 또는 work 폴더 선택
- 🗑️ 캐시 삭제: PHP 색인 + 문자열 검색 캐시 클리어
- 📥 내보내기: JSON/CSV 형식으로 색인 데이터 내보내기

# 추가 기능 (탭 메뉴)
- 💾 심볼 검색: 함수, 클래스, 메서드 검색
- 📊 호출 그래프: 함수 호출 관계 시각화 + 💻 코드 표시
- 🔗 의존성 분석: 순환 의존성, 호출 체인 분석
```

#### ✨ NEW: Graph 탭에서 함수 구현 코드 표시 (v2.5.0+)
Graph 탭에서 함수/메서드를 클릭하면 우측 패널이나 팝업에 **실제 구현 코드가 표시**됩니다!

**주요 개선사항**:
- 📝 **정확한 코드 표시**: 1,468 바이트 autolink5 함수 완전히 표시
- 🔍 **메서드도 지원**: 클래스 메서드 4,063개 추가 인덱싱
- 📂 **.inc 파일 지원**: work/mobile의 37개 .inc 파일 처리
- ⚡ **빠른 조회**: code-index.json으로 5-10ms 응답 (기존 500-800ms)
- 💾 **3단계 캐싱**: L1(메모리) <1ms, L2(파일) 15-20ms, L3(색인) 100-300ms

**기술 개선**:
- ✅ Word boundary로 함수명 정확하게 구분 (autolink5 vs autolink5_callback)
- ✅ 라인 번호 정확성 개선 (주석 제거로 인한 오류 해결)
- ✅ 별도 code-index.json으로 함수 본문 저장 (9.3MB)
- ✅ PHPIndexService에서 code-index 지원

**사용 예시**:
1. Dashboard 접속 → 💾 심볼 검색 또는 📊 호출 그래프 탭
2. 함수명 클릭 (예: autolink5)
3. 우측 패널의 💻 코드 탭에 구현 코드 표시
4. 코드 보기/복사/팝업/파일 열기 등 지원

자세한 내용은 [IMPLEMENTATION.md](./IMPLEMENTATION.md) 참고
- 🔍 SQL 쿼리 추출: PHP 파일의 SQL 쿼리 자동 추출
- 🔍 문자열 검색: 코드의 특정 문자열 위치 추적
```

**주요 기능**:
- 🔧 **색인 관리가 대시보드 기본 화면** (v2.4.1+)
- 📁 소스 폴더 동적 선택 및 재색인
- ⚡ **code-index.json 지원** (50-100배 빠른 코드 조회)
- 🔍 PHP 심볼 인덱싱 및 검색
...
|------|------|
| **프로젝트 시작** | 2026-03-13 |
| **마지막 업데이트** | 2026-03-26 (v2.3.4) |
| **주요 기여** | Claude (AI-assisted development) |
| **상태** | 🟢 정상 운영 중 |

### 🔄 버전 히스토리

**v2.3.4** (2026-03-26) - 포트 관리 및 소스 폴더 변경 기능 추가
- ✨ Windows 포트 관리 스크립트 (start-server.bat, stop-server.bat)
- ✨ 소스 폴더 경로 변경 기능 (색인 관리 패널)
- ✨ 경로 입력 필드로 새로운 PHP 폴더 색인화 가능
- 📚 USAGE.md 사용 설명서 추가

**v2.3.3** (2026-03-26) - 색인 관리 패널 UI 개선
- ✨ 색인 파일 위치와 소스 폴더 위치 분리 표시
- ✨ 위치 정보 박스에 강조 스타일링 추가
- ✨ 반응형 디자인 (모바일 1열, 데스크톱 2열)
- ✨ 현재 색인 정보들 명확하게 구성

**v2.3.2** (2026-03-26) - 로딩 인디케이터 버그 수정
- 🐛 모든 페이지에서 로딩 표시가 사라지지 않는 문제 해결
- ✅ CSS 충돌 해결 (components.css display 속성)
- ✅ app.js에서 명시적 로딩 표시 제거 추가
